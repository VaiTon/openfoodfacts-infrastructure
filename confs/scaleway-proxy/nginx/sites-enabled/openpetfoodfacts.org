...
# create zone "opff" for rate limiting 
limit_req_zone $binary_remote_addr zone=opff:10m rate=240r/m;
...
server {
    ...
    # ssl stapling (OCSP) no longer supported by letsencrypt
    # ssl_stapling on;
    ...
    access_log off;
    # logs errors location
    access_log  /var/log/nginx/openpetfoodfacts.org.access-errors.log  main buffer=256K flush=1s if=$error_log;
    error_log   /var/log/nginx/openpetfoodfacts.org.errors.log;
    ...
    location ~ ^/(css/|js/|fonts/|images/(attributes|favicon|icons|illustrations|lang|logos|misc|panels|svg)/|.well-known/|api/v./(preferences|attribute_groups)|data/i18n) {
        ...
        proxy_pass http://10.13.1.115:80;
...
#        proxy_cache_valid any 5s;
	...
	# proxy_buffering off disables caching
 	...
 	proxy_intercept_errors on;
        ...
        # Rate limiting
        limit_req zone=opff burst=120 nodelay; # burst=120 means you can do 240+120 requests
