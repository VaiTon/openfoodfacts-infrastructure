...
Instead we use [syncoid / sanoid](./software/sanoid.md) to snapshot and synchronize data to other servers.
...
We don't use standard proxmox replication of storages, because it is incompatible with using [syncoid / sanoid](./software/sanoid.md), as it removes snapshots on destination and does not allow to choose destination location.
